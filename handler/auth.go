...
import (
	...
	"github.com/alexohneander/zeitungsfritze/config"
...
func Login(c *fiber.Ctx) error {
	...
	if err := c.BodyParser(&input); err != nil {
		log.Info().Msgf("Error on login request: %s", err)
	...
	if valid(identity) {
		...
		if err != nil {
			log.Info().Msgf("User %s tried to login but not found", identity)
	...
	if !CheckPasswordHash(pass, ud.Password) {
		log.Info().Msgf("User %s tried to login with wrong password", ud.Username)
	...
	log.Info().Msgf("User %s logged in", ud.Username)
